...
def compute_derivatives(position_series, fps):
    ...
    smooth = savgol_filter(position_series, window_length=7, polyorder=2)
    speed = np.gradient(smooth) * fps
    acceleration = np.gradient(speed) * fps
    jerk = np.gradient(acceleration) * fps
...
def process_video(VIDEO_PATH, LABEL=0):
    ...
    for i in range(33):
        ...
        v = np.array(trajectory[i]['visibility'])

        # ✨ 낮은 visibility 제거 + 보간
        x[v < 0.5] = np.nan
        y[v < 0.5] = np.nan
        x = pd.Series(x).interpolate(limit_direction='both').fillna(method='bfill').fillna(method='ffill').values
        y = pd.Series(y).interpolate(limit_direction='both').fillna(method='bfill').fillna(method='ffill').values
