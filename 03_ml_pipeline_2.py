...
def feature_selection(X, y, relieff_threshold=0.01, rf_cumulative_importance=0.95):
    ...
    relieff_mask = relieff_scores > relieff_threshold
    relieff_indices = np.where(relieff_mask)[0]
    relieff_features = [feature_names[i] for i in relieff_indices]

    print(f"   ReliefF 선택: {len(relieff_features)}개 / 평균 score: {relieff_scores[relieff_indices].mean():.4f}")

    if len(relieff_features) < 5:
        print("⚠️ 선택된 특성이 너무 적어 후속 단계 생략")
    ...
    rf_model.fit(X_relieff, y)

    rf_importances = rf_model.feature_importances_
    sorted_indices = np.argsort(rf_importances)[::-1]
    cumsum = np.cumsum(rf_importances[sorted_indices])
    n_keep = np.argmax(cumsum >= rf_cumulative_importance) + 1

    rf_indices = sorted_indices[:n_keep]
    rf_features = [relieff_features[i] for i in rf_indices]
    X_rf = X_relieff[rf_features]

    print(f"   RF 선택: {len(rf_features)}개 / 누적 중요도: {cumsum[n_keep - 1]:.4f}")

    if len(rf_features) < 5 or len(y) < 4:
    ...
    final_features = [rf_features[i] for i, selected in enumerate(rfecv.support_) if selected]
    print(f"\n✅ 최종 선택된 특성 수: {len(final_features)}개")
...
def run_all():
    ...
    # 특성 선택 (Train 데이터만 사용) - 최소 특성만 자동 선택
    selected_features = feature_selection(X_train, y_train)
    X_train_selected = X_train[selected_features]
    X_test_selected = X_test[selected_features]

    # ✅ 결측치 처리 (feature selection 이후에도 필요)
    print(f"\n🔧 결측치 처리")
    print(f"   Train 결측치: {X_train_selected.isnull().sum().sum()}개")
    print(f"   Test 결측치: {X_test_selected.isnull().sum().sum()}개")

    if X_train_selected.isnull().sum().sum() > 0:
        print("   → Train 데이터 결측치를 중앙값으로 대체")
        X_train_selected = X_train_selected.fillna(X_train_selected.median())

    if X_test_selected.isnull().sum().sum() > 0:
        print("   → Test 데이터 결측치를 Train 중앙값으로 대체")
        X_test_selected = X_test_selected.fillna(X_train_selected.median())

    # 무한대 값 처리
    X_train_selected = X_train_selected.replace([np.inf, -np.inf], np.nan).fillna(X_train_selected.median())
    X_test_selected = X_test_selected.replace([np.inf, -np.inf], np.nan).fillna(X_train_selected.median())

    print(f"   ✅ 처리 후 - Train 결측치: {X_train_selected.isnull().sum().sum()}개")
    print(f"   ✅ 처리 후 - Test 결측치: {X_test_selected.isnull().sum().sum()}개")

    all_results = []
    tuning_methods = [TuningMethod.DEFAULT, TuningMethod.OPTUNA]
